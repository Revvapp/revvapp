...
export default function DetailerOnboardingSignupScreen() {
  ...
  const [accountConflict, setAccountConflict] = useState<'client' | 'detailer' | 'unknown' | null>(null);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      router.replace('/detailer/onboarding/step1');
    } catch (e) {
      if (isNetworkError(e)) {
      ...
      if (isEmailInUse(e)) {
        ...
        try {
          ...
          setAccountConflict(
            existing === 'client' ? 'client' : existing === 'detailer' ? 'detailer' : 'unknown'
          );
        } catch {
        ...
        >
          ...
          <View style={styles.headerRow}>
            ...
            <View style={styles.stepBadge}>
              <Text style={styles.stepBadgeText}>Step 0 of 4</Text>
            </View>
          </View>

          <Text style={styles.heroWhite}>CREATE YOUR</Text>
          <Text style={styles.heroGold}>REVV PRO ACCOUNT</Text>
          <Text style={styles.heroSub}>
            Join the premium detailing network. Your first 60 days are on us.
          </Text>

          <View style={styles.foundingBanner}>
            <View style={styles.foundingRow}>
              <Ionicons name="star" size={18} color={COLORS.gold} />
              <Text style={styles.foundingTitle}>FOUNDING PRO OFFER</Text>
            </View>
            <Text style={styles.foundingBody}>
              60-day free trial · No card required · Permanent Founding Pro badge
            </Text>
          </View>
          ...
          {accountConflict === 'client' && (
            <View style={styles.conflictCard}>
              <Text style={styles.conflictText}>
                This email is registered as a car owner account. Would you like to log in as a car owner instead?
              </Text>
              <Pressable style={styles.conflictPrimary} onPress={() => router.push('/login')}>
                <Text style={styles.conflictPrimaryText}>LOG IN AS CAR OWNER</Text>
              </Pressable>
              <Pressable
          ...
          {accountConflict === 'detailer' && (
            <View style={styles.conflictCard}>
              <Text style={styles.conflictText}>You already have a Revv Pro account. Log in instead?</Text>
              <Pressable style={styles.conflictPrimary} onPress={() => router.push('/login')}>
          ...
          <Pressable style={styles.cta} onPress={handleSignup} disabled={loading || accountConflict != null}>
            {loading ? (
              ...
              <Text style={styles.ctaText}>CREATE MY REVV PRO ACCOUNT</Text>
            )}
          </Pressable>
